...
namespace ElasticAnalysis2
{
    public class ElasticAnalysis2 : GH_Component
    {
        public ElasticAnalysis2()
          : base("Analysis using OpenSees(considering orthotropic material)", "Elastic Analysis(considering orthotropic material)",
              "Analysis using OpenSees.NET(considering orthotropic material)",
              "OpenSees", "Analysis")
        {
        }
        protected override void RegisterInputParams(GH_Component.GH_InputParamManager pManager)
        {
            ...
            pManager.AddNumberParameter("Young's mudulus", "E", "[...](DataList)", GH_ParamAccess.tree, 2.1e+8);///
            pManager.AddNumberParameter("Shear modulus", "poi", "[...](DataList)", GH_ParamAccess.tree, 0.3);///
        ...
        protected override void RegisterOutputParams(GH_Component.GH_OutputParamManager pManager)
        {
            ...
            pManager.AddNumberParameter("stress(shell)", "shell_f", "[[Ni,Qyi,Qzi,Mxi,Myi,Mzi,Ni,Qyi,Qzi,Mxj,Myj,Mzj,Nk,Qyk,Qzk,Mxk,Myk,Mzk,Nl,Qyl,Qzl,Mxl,Myl,Mzl],...](DataTree)", GH_ParamAccess.tree);///8
        ...
        protected override void SolveInstance(IGH_DataAccess DA)
        {
                ...
                for (int e = 0; e < m; e++)
                {
                    ...
                    if (joint_No.Contains(e))
                    {
                        ...
                        if ((int)joint[k][1].Value == 0)
                        {
                            ...
                            theDomain.AddElement(new OpenSees.Elements.ElasticBeam3dWrapper(e + 1, A[sec], E[mat][0].Value, E[mat][0].Value / 2.0 / (1 + poi[mat][0].Value), J[sec], Iy[sec], Iz[sec], n + nc, j + 1, geo, 0, 0, e + 1));
                        ...
                        else if ((int)joint[k][1].Value == 1)
                        {
                            ...
                            theDomain.AddElement(new OpenSees.Elements.ElasticBeam3dWrapper(e + 1, A[sec], E[mat][0].Value, E[mat][0].Value / 2.0 / (1 + poi[mat][0].Value), J[sec], Iy[sec], Iz[sec], i + 1, n + nc, geo, 0, 0, e + 1));
                        ...
                        else if ((int)joint[k][1].Value == 2)
                        {
                            ...
                            theDomain.AddElement(new OpenSees.Elements.ElasticBeam3dWrapper(e + 1, A[sec], E[mat][0].Value, E[mat][0].Value / 2.0 / (1 + poi[mat][0].Value), J[sec], Iy[sec], Iz[sec], n + nc - 1, n + nc, geo, 0, 0, e + 1));
                            new_ij.Add(new List<int>() { n + nc - 1, n + nc });
                        }
                        else
                        {
                            theDomain.AddElement(new OpenSees.Elements.ElasticBeam3dWrapper(e + 1, A[sec], E[mat][0].Value, E[mat][0].Value / 2.0 / (1 + poi[mat][0].Value), J[sec], Iy[sec], Iz[sec], i + 1, j + 1, geo, 0, 0, e + 1));
            ...
            if (_ijkl.Branches[0][0].Value != -9999)
            {
                for (int e = 0; e < m2; e++)
                {
                    ...
                    if (l != -1)
                    {
                        if (E[mat].Count == 1)
                        {
                            var shellmaterial = new OpenSees.Materials.Sections.ElasticMembranePlateSectionWrapper(e + 1, E[mat][0].Value, poi[mat][0].Value, t, 0);
                        ...
                        else if (E[mat].Count == 3)
                        {
                            var ndmat = new OpenSees.Materials.NDMaterials.ElasticOrthotropicThreeDimensionalWrapper(mat + 1, E[mat][0].Value, E[mat][1].Value, E[mat][2].Value, poi[mat][0].Value, poi[mat][1].Value, poi[mat][2].Value, E[mat][0].Value / 2.0 / (1 + poi[mat][0].Value), E[mat][1].Value / 2.0 / (1 + poi[mat][1].Value), E[mat][2].Value / 2.0 / (1 + poi[mat][2].Value), 0);
                            var shellmaterial = new OpenSees.Materials.Sections.MembranePlateFiberSectionWrapper(e + 1, t, ndmat);
                            var ele = new OpenSees.Elements.ShellDKGQWrapper(m + mc + e + 1, i + 1, j + 1, k + 1, l + 1, shellmaterial);
                            theDomain.AddElement(ele); shell_elements.Add(ele);
                        }
                    }
            ...
            if (_ijkl.Branches[0][0].Value != -9999)
            {
                for (int e = 0; e < m2; e++)
                {
                    ...
                    //var Kmat = new Matrix(24, 24); var Dmat = new Matrix(24, 1);
                    //for (int i = 0; i < 24; i++)
                    //{
                    //    Dmat[i,0] = dlist[i].Value;
                    //    for (int j = 0; j < 24; j++)
                    //    {
                    //        Kmat[i, j] = K[i, j];
                    //    }
                    //}
                    //var fe = Kmat * Dmat;
                    //for (int i = 0; i < 24; i++) { flist.Add(new GH_Number(fe[i,0])); }
            ...
            if (option[0] == 1 && _ij.Branches[0][0].Value != -9999)
            {
                ...
                else
                {
                    ...
                    for (int e = 0; e < ij.Count; e++)
                    {
                        ...
                        var f_ei = element_sectional_force(ue, l, 0, E[mat][0].Value, E[mat][0].Value / 2.0 / (1 + poi[mat][0].Value), A[sec], Iy[sec], Iz[sec], J[sec]);
                        var f_ej = element_sectional_force(ue, l, l, E[mat][0].Value, E[mat][0].Value / 2.0 / (1 + poi[mat][0].Value), A[sec], Iy[sec], Iz[sec], J[sec]);
                        var f_ec = element_sectional_force(ue, l, l / 2.0, E[mat][0].Value, E[mat][0].Value / 2.0 / (1 + poi[mat][0].Value), A[sec], Iy[sec], Iz[sec], J[sec]);
        ...
        protected override System.Drawing.Bitmap Icon
        {
            get
            {
                ...
                return OpenSeesUtility.Properties.Resources.ElasticAnalysis2;
            }
        }
        ...
        {
            get { return new Guid("64d9294a-168c-4cf8-9fba-1d8be53bcd93"); }
        }
    }
}