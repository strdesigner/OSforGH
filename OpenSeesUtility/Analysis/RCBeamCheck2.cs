...
{
    public class RCBeamCheck2 : GH_Component
    {
        ...
        public RCBeamCheck2()
          : base("Allowable stress design for RC beams(1 bar per 1 element)", "RCBeamCheck2",
              "Allowable stress design(danmensantei) for RC beams using Japanese Design Code(1 bar per 1 element)",
              "OpenSees", "Analysis")
        {
        }
        ...
        {
            pManager.AddTextParameter("layer(RCbeam)", "layer(RCbeam)", "[layername1,layername2,...](Datalist)", GH_ParamAccess.list);
        ...
        {
            List<string> layer = new List<string>(); DA.GetDataList("layer(RCbeam)", layer); var acc = 5e-3; DA.GetData("accuracy", ref acc);
            ...
            if (r[0][0].Value != -9999 && ij[0][0].Value != -9999 && sec_f[0][0].Value != -9999 && barT1[0][0].Value != -9999 && barT2[0][0].Value != -9999 && barB1[0][0].Value != -9999 && barB2[0][0].Value != -9999 && P1[0] != -9999 && P2[0] != -9999)
            {
                ...
                var doc = RhinoDoc.ActiveDoc; List<Curve> lines = new List<Curve>();
                ...
                for (int ind = 0; ind < index.Count; ind++)
                {
                    ...
                    {
                        var r1 = lines[j].PointAtStart; var r2 = lines[j].PointAtEnd;
                        var l0 = lines[j].GetLength();
                        var l1 = (r1 - ri).Length; var l2 = (r2 - ri).Length;
                        var l3 = (r1 - rj).Length; var l4 = (r2 - rj).Length;
                        if (Math.Abs(l0 - l1 - l2) < acc && Math.Abs(l0 - l3 - l4) < acc)//まず分割前の直線を探索
                        {
                            var icj = new List<int>();
                            var r12 = (r1 * 2 + r2) / 3.0; var r21 = (r1 + r2 * 2) / 3.0;//分割前の直線の3分割点
                            var rc = (ri + rj) / 2.0;//要素の中点
                            var l5 = (ri - r12).Length; var l6= (ri - r21).Length;
                            if (Math.Abs(l0 / 3.0 - l1 - l5) < acc) { icj.Add(0); }
                            else if (Math.Abs(l0 / 3.0 - l5 - l6) < acc) { icj.Add(1); }
                            else if (Math.Abs(l0 / 3.0 - l2 - l6) < acc) { icj.Add(2); }
                            var l7 = (r1 - rc).Length; var l8 = (r2 - rc).Length; var l9 = (r12 - rc).Length; var l10 = (r21 - rc).Length;
                            if (Math.Abs(l0 / 3.0 - l7 - l9) < acc) { icj.Add(0); }
                            else if (Math.Abs(l0 / 3.0 - l9 - l10) < acc) { icj.Add(1); }
                            else if (Math.Abs(l0 / 3.0 - l8 - l10) < acc) { icj.Add(2); }
                            var l11 = (r12 - rj).Length; var l12 = (r21 - rj).Length;
                            if (Math.Abs(l0 / 3.0 - l3 - l11) < acc) { icj.Add(0); }
                            else if (Math.Abs(l0 / 3.0 - l11 - l12) < acc) { icj.Add(1); }
                            else if (Math.Abs(l0 / 3.0 - l4 - l12) < acc) { icj.Add(2); }
                            icjlist.Add(icj);
                        }
                    }
                }
                ...
                for (int ind = 0; ind < index.Count; ind++)
                {
                    ...
                    var Ma_tL = new List<double> { Ma_itL, Ma_ctL, Ma_jtL }; var Ma_bL = new List<double> { Ma_ibL, Ma_cbL, Ma_jbL };
                    if (Mi < 0)
                    {
                        klist.Add(new GH_Number(Math.Abs(Mi) / Ma_tL[icjlist[ind][0]]));
                    }
                    else
                    {
                        klist.Add(new GH_Number(Math.Abs(Mi) / Ma_bL[icjlist[ind][0]]));
                    }
                    if (Mj > 0)
                    {
                        klist.Add(new GH_Number(Math.Abs(Mj) / Ma_tL[icjlist[ind][2]]));
                    }
                    else
                    {
                        klist.Add(new GH_Number(Math.Abs(Mj) / Ma_bL[icjlist[ind][2]]));
                    }
                    if (Mc < 0)
                    {
                        klist.Add(new GH_Number(Math.Abs(Mc) / Ma_tL[icjlist[ind][1]]));
                    }
                    else
                    {
                        klist.Add(new GH_Number(Math.Abs(Mc) / Ma_bL[icjlist[ind][1]]));
                    }
                    var Qa_L = new List<double> { Qa_iL, Qa_cL, Qa_jL };
                    klist.Add(new GH_Number(Math.Abs(Qi) / Qa_L[icjlist[ind][0]])); klist.Add(new GH_Number(Math.Abs(Qj) / Qa_L[icjlist[ind][2]])); klist.Add(new GH_Number(Math.Abs(Qc) / Qa_L[icjlist[ind][1]]));
                    //kentei.AppendRange(klist, new GH_Path(e,0));
                    var Ma_tS = new List<double> { Ma_itS, Ma_ctS, Ma_jtS }; var Ma_bS = new List<double> { Ma_ibS, Ma_cbS, Ma_jbS };
                    var ki = new List<double>(); var kj = new List<double>(); var kc = new List<double>();
                    if (Mi + Mi_x < 0) { ki.Add(Math.Abs(Mi + Mi_x) / Ma_tS[icjlist[ind][0]]); }
                    else { ki.Add(Math.Abs(Mi + Mi_x) / Ma_bS[icjlist[ind][0]]); }
                    if (Mi + Mi_x2 < 0) { ki.Add(Math.Abs(Mi + Mi_x2) / Ma_tS[icjlist[ind][0]]); }
                    else { ki.Add(Math.Abs(Mi + Mi_x2) / Ma_bS[icjlist[ind][0]]); }
                    if (Mi + Mi_y < 0) { ki.Add(Math.Abs(Mi + Mi_y) / Ma_tS[icjlist[ind][0]]); }
                    else { ki.Add(Math.Abs(Mi + Mi_y) / Ma_bS[icjlist[ind][0]]); }
                    if (Mi + Mi_y2 < 0) { ki.Add(Math.Abs(Mi + Mi_y2) / Ma_tS[icjlist[ind][0]]); }
                    else { ki.Add(Math.Abs(Mi + Mi_y2) / Ma_bS[icjlist[ind][0]]); }
                    if (Mj + Mj_x > 0) { kj.Add(Math.Abs(Mj + Mj_x) / Ma_tS[icjlist[ind][2]]); }
                    else { kj.Add(Math.Abs(Mj + Mj_x) / Ma_bS[icjlist[ind][2]]); }
                    if (Mj + Mj_x2 > 0) { kj.Add(Math.Abs(Mj + Mj_x2) / Ma_tS[icjlist[ind][2]]); }
                    else { kj.Add(Math.Abs(Mj + Mj_x2) / Ma_bS[icjlist[ind][2]]); }
                    if (Mj + Mj_y > 0) { kj.Add(Math.Abs(Mj + Mj_y) / Ma_tS[icjlist[ind][2]]); }
                    else { kj.Add(Math.Abs(Mj + Mj_y) / Ma_bS[icjlist[ind][2]]); }
                    if (Mj + Mj_y2 > 0) { kj.Add(Math.Abs(Mj + Mj_y2) / Ma_tS[icjlist[ind][2]]); }
                    else { kj.Add(Math.Abs(Mj + Mj_y2) / Ma_bS[icjlist[ind][2]]); }
                    if (Mc + Mc_x < 0) { kc.Add(Math.Abs(Mc + Mc_x) / Ma_tS[icjlist[ind][1]]); }
                    else { kc.Add(Math.Abs(Mc + Mc_x) / Ma_bS[icjlist[ind][1]]); }
                    if (Mc + Mc_x2 < 0) { kc.Add(Math.Abs(Mc + Mc_x2) / Ma_tS[icjlist[ind][1]]); }
                    else { kc.Add(Math.Abs(Mc + Mc_x2) / Ma_bS[icjlist[ind][1]]); }
                    if (Mc + Mc_y < 0) { kc.Add(Math.Abs(Mc + Mc_y) / Ma_tS[icjlist[ind][1]]); }
                    else { kc.Add(Math.Abs(Mc + Mc_y) / Ma_bS[icjlist[ind][1]]); }
                    if (Mc + Mc_y2 < 0) { kc.Add(Math.Abs(Mc + Mc_y2) / Ma_tS[icjlist[ind][1]]); }
                    else { kc.Add(Math.Abs(Mc + Mc_y2) / Ma_bS[icjlist[ind][1]]); }
                    ...
                    var Qa_LpX = new List<double> { Qa_iLpX, Qa_cLpX, Qa_jLpX }; var Qa_LmX = new List<double> { Qa_iLmX, Qa_cLmX, Qa_jLmX };
                    var Qa_LpY = new List<double> { Qa_iLpY, Qa_cLpY, Qa_jLpY }; var Qa_LmY = new List<double> { Qa_iLmY, Qa_cLmY, Qa_jLmY };
                    k2list.Add(new GH_Number(Math.Max(Math.Max(Math.Abs(Qi + Qi_x * N) / Qa_LpX[icjlist[ind][0]], Math.Abs(Qi + Qi_x2 * N) / Qa_LmX[icjlist[ind][0]]), Math.Max(Math.Abs(Qi + Qi_y * N) / Qa_LpY[icjlist[ind][0]], Math.Abs(Qi + Qi_y2 * N) / Qa_LpY[icjlist[ind][0]]))));
                    k2list.Add(new GH_Number(Math.Max(Math.Max(Math.Abs(Qj + Qj_x * N) / Qa_LpX[icjlist[ind][2]], Math.Abs(Qj + Qj_x2 * N) / Qa_LmX[icjlist[ind][2]]), Math.Max(Math.Abs(Qj + Qj_y * N) / Qa_LpY[icjlist[ind][2]], Math.Abs(Qj + Qj_y2 * N) / Qa_LpY[icjlist[ind][2]]))));
                    k2list.Add(new GH_Number(Math.Max(Math.Max(Math.Abs(Qc + Qc_x * N) / Qa_LpX[icjlist[ind][1]], Math.Abs(Qc + Qc_x2 * N) / Qa_LmX[icjlist[ind][1]]), Math.Max(Math.Abs(Qc + Qc_y * N) / Qa_LpY[icjlist[ind][1]], Math.Abs(Qc + Qc_y2 * N) / Qa_LpY[icjlist[ind][1]]))));
                ...
                if (on_off == 1)
                {
                    ...
                    if (sec_f[0].Count == 18)
                    {
                        ...
                        for (int ind = 0; ind < index.Count; ind++)//
                        {
                            ...
                            var Ftt = new List<List<string>> { Ftit[ind], Ftct[ind], Ftjt[ind] };
                            var Ftb = new List<List<string>> { Ftib[ind], Ftcb[ind], Ftjb[ind] };
                            var Fs = new List<List<string>> { Fsi[ind], Fsc[ind], Fsj[ind] };
                            values.Add(Ftt[icjlist[ind][0]]);
                            values.Add(Ftb[icjlist[ind][0]]);
                            values.Add(Ftt[icjlist[ind][1]]);
                            values.Add(Ftb[icjlist[ind][1]]);
                            values.Add(Ftt[icjlist[ind][2]]);
                            values.Add(Ftb[icjlist[ind][2]]);
                            values.Add(Fs[icjlist[ind][0]]); values.Add(Fs[icjlist[ind][1]]); values.Add(Fs[icjlist[ind][2]]);
                            values.Add(FC[ind]); values.Add(FS[ind]); values.Add(Nod[ind]);
                            var Bart2 = new List<string> { Bart[ind][icjlist[ind][0]], Bart[ind][icjlist[ind][1]], Bart[ind][icjlist[ind][2]] };
                            var Dt2 = new List<string> { Dt[ind][icjlist[ind][0]], Dt[ind][icjlist[ind][1]], Dt[ind][icjlist[ind][2]] };
                            var Barb2 = new List<string> { Barb[ind][icjlist[ind][0]], Barb[ind][icjlist[ind][1]], Barb[ind][icjlist[ind][2]] };
                            var Db2 = new List<string> { Db[ind][icjlist[ind][0]], Db[ind][icjlist[ind][1]], Db[ind][icjlist[ind][2]] };
                            var Bars2 = new List<string> { Bars[ind][icjlist[ind][0]], Bars[ind][icjlist[ind][1]], Bars[ind][icjlist[ind][2]] };
                            values.Add(Bart2);
                            values.Add(Dt2);
                            values.Add(Barb2);
                            values.Add(Db2);
                            ...
                            var Mait = Mt_aL[ind][icjlist[ind][0]]; var Mact = Mt_aL[ind][icjlist[ind][1]]; var Majt = Mt_aL[ind][icjlist[ind][2]];
                            var Maib = Mb_aL[ind][icjlist[ind][0]]; var Macb = Mb_aL[ind][icjlist[ind][1]]; var Majb = Mb_aL[ind][icjlist[ind][2]];
                            ...
                            var Qai = Q_aL[ind][icjlist[ind][0]]; var Qac = Q_aL[ind][icjlist[ind][1]]; var Qaj = Q_aL[ind][icjlist[ind][2]];
                    ...
                    else if (sec_f[0].Count == 18 * 3 || sec_f[0].Count == 18 * 5)
                    {
                        ...
                        for (int ind = 0; ind < index.Count; ind++)//
                        {
                            ...
                            Mait = Mt_aS[ind][icjlist[ind][0]]; Mact = Mt_aS[ind][icjlist[ind][1]]; Majt = Mt_aS[ind][icjlist[ind][2]];
                            Maib = Mb_aS[ind][icjlist[ind][0]]; Macb = Mb_aS[ind][icjlist[ind][1]]; Majb = Mb_aS[ind][icjlist[ind][2]];
                            ...
                            Qai = Q_aLpX[ind][icjlist[ind][0]]; Qac = Q_aLpX[ind][icjlist[ind][1]]; Qaj = Q_aLpX[ind][icjlist[ind][2]];
                            ...
                            Qc_text = Qc.ToString("F10").Substring(0, Math.Max(4, Digit((int)Qc)));
                            Qj_text = Qj.ToString("F10").Substring(0, Math.Max(4, Digit((int)Qj)));
                            Qai = Q_aLpY[ind][icjlist[ind][0]]; Qac = Q_aLpY[ind][icjlist[ind][1]]; Qaj = Q_aLpY[ind][icjlist[ind][2]];
                            Qai_text = Qai.ToString("F10").Substring(0, Math.Max(4, Digit((int)Qai)));
                            Qac_text = Qac.ToString("F10").Substring(0, Math.Max(4, Digit((int)Qac)));
                            Qaj_text = Qaj.ToString("F10").Substring(0, Math.Max(4, Digit((int)Qaj)));
                            ki2 = Math.Abs(Qi / Qai); kc2 = Math.Abs(Qc / Qac); kj2 = Math.Abs(Qj / Qaj);
                            ...
                            Qc_text = Qc.ToString("F10").Substring(0, Math.Max(4, Digit((int)Qc)));
                            Qj_text = Qj.ToString("F10").Substring(0, Math.Max(4, Digit((int)Qj)));
                            Qai = Q_aLmX[ind][icjlist[ind][0]]; Qac = Q_aLmX[ind][icjlist[ind][1]]; Qaj = Q_aLmX[ind][icjlist[ind][2]];
                            Qai_text = Qai.ToString("F10").Substring(0, Math.Max(4, Digit((int)Qai)));
                            Qac_text = Qac.ToString("F10").Substring(0, Math.Max(4, Digit((int)Qac)));
                            Qaj_text = Qaj.ToString("F10").Substring(0, Math.Max(4, Digit((int)Qaj)));
                            ki2 = Math.Abs(Qi / Qai); kc2 = Math.Abs(Qc / Qac); kj2 = Math.Abs(Qj / Qaj);
                            ...
                            Qc_text = Qc.ToString("F10").Substring(0, Math.Max(4, Digit((int)Qc)));
                            Qj_text = Qj.ToString("F10").Substring(0, Math.Max(4, Digit((int)Qj)));
                            Qai = Q_aLmY[ind][icjlist[ind][0]]; Qac = Q_aLmY[ind][icjlist[ind][1]]; Qaj = Q_aLmY[ind][icjlist[ind][2]];
                            Qai_text = Qai.ToString("F10").Substring(0, Math.Max(4, Digit((int)Qai)));
                            Qac_text = Qac.ToString("F10").Substring(0, Math.Max(4, Digit((int)Qac)));
                            Qaj_text = Qaj.ToString("F10").Substring(0, Math.Max(4, Digit((int)Qaj)));
                            ki2 = Math.Abs(Qi / Qai); kc2 = Math.Abs(Qc / Qac); kj2 = Math.Abs(Qj / Qaj);
        ...
        protected override System.Drawing.Bitmap Icon
        {
            get
            {
                ...
                return OpenSeesUtility.Properties.Resources.rcbeamcheck2;
            }
        }
        ...
        {
            get { return new Guid("2c42c1db-564c-4773-a72a-2e7cb594f59a"); }
        }
