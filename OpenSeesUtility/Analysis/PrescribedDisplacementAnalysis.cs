...
namespace OpenSeesUtility.Analysis
{
    public class PrescribedDisplacementAnalysis : GH_Component
    {
        /// <summary>
        /// Initializes a new instance of the PrescribedDisplacementAnalysis class.
        /// </summary>
        public PrescribedDisplacementAnalysis()
          : base("PrescribedDisplacementAnalysis", "Prescribed Displacement Analysis",
              "Prescribed Displacement Analysis using OpenSees.NET",
              "OpenSees", "Analysis")
        {
        }
        ...
        protected override void RegisterInputParams(GH_Component.GH_InputParamManager pManager)
        {
            ...
            pManager.AddNumberParameter("prescribed displacement", "Disp", "[[node No.,DOF,value],...](DataTree)", GH_ParamAccess.tree, -9999);///
            ...
            pManager.AddIntegerParameter("option", "option", "[0 or 1](DataList) when 1, sec_f is calculated ", GH_ParamAccess.list, new List<int> { 1 });///
        }
        ...
        protected override void RegisterOutputParams(GH_Component.GH_OutputParamManager pManager)
        {
            ...
            pManager.AddNumberParameter("nodal displacement (each node)", "D(R)", "[[dxi,dyi,dzi,theta_xi,theta_yi,theta_zi,dxj,dyj,dzj,theta_xj,theta_yj,theta_zj],...](DataTree)", GH_ParamAccess.tree);///13
        }
        /// <summary>
        ...
        {
            IList<List<GH_Number>> r; IList<List<GH_Number>> ij; IList<List<GH_Number>> ijkl; IList<List<GH_Number>> pred_v; List<Vector3d> l_vec = new List<Vector3d>(); IList<List<GH_Number>> rigid;
            ...
            if (!DA.GetDataTree("prescribed displacement", out GH_Structure<GH_Number> _d_v)) { }
            else if (_d_v.Branches[0][0].Value != -9999)
            {
                pred_v = _d_v.Branches; nf = pred_v.Count;
                //var theSeries = new OpenSees.Components.Timeseries.ConstantSeriesWrapper(1);
                ...
                for (int i = 0; i < nf; i++)
                {
                    int j = (int)pred_v[i][0].Value; int k = (int)pred_v[i][1].Value; double pred = (double)pred_v[i][2].Value;
                    theDomain.AddSP_Constraint(new OpenSees.Components.Constraints.SP_ConstraintWrapper(j + 1, k, pred, true));
                    //RhinoApp.WriteLine(pred.ToString());
                }
            }
            ...
            ij = _ij.Branches;
            var f_i = new double[] { 0, 0, 0, 0, 0, 0 }; var f_j = new double[] { 0, 0, 0, 0, 0, 0 }; var f_c = new double[] { 0, 0, 0, 0, 0, 0 };
        ...
        protected override System.Drawing.Bitmap Icon
        {
            get
            {
                ...
                return OpenSeesUtility.Properties.Resources.PrescribedDisplacementAnalysis;
            }
        }
        ...
        {
            get { return new Guid("25ED2EB4-6DA4-4D9D-96E2-A8A17572A6F8"); }
        }
    }
}