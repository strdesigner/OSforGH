...
namespace Kabecheck2025
{
    public class Kabecheck2025 : GH_Component
    {
        ...
        public Kabecheck2025()
          : base("Checking KABERYOU2025", "KabeCheck2025",
              "KABERYOU calculation based on new Japanese Design Code (since 2025-)",
              "OpenSees", "Analysis")
        {
        }
        ...
        protected override void RegisterInputParams(GH_Component.GH_InputParamManager pManager)
        {
            ...
            pManager.AddNumberParameter("for eathquake", "for eathquake", "[for 1F, for 2F, for 3F]", GH_ParamAccess.list, new List<double> { 81.0, 62.0, 36.0 });//13
            pManager.AddNumberParameter("max W", "max W", "The upper limit of the kaberyou", GH_ParamAccess.item, 7.0);//14
            pManager.AddNumberParameter("for wind", "for wind", "normally 50 or 75", GH_ParamAccess.item, 50.0);//15
            pManager.AddNumberParameter("p", "p", "Result output position", GH_ParamAccess.list, new List<double> { 0, 0, 0 });//16
            pManager.AddNumberParameter("fontsize", "fontsize", "fontsize", GH_ParamAccess.item, 12.0);//17
            pManager.AddTextParameter("outputname", "outputname", "output file name", GH_ParamAccess.item, "wkabe");//18
            ...
            if (flayer1.Count != 0 && wlayer1.Count != 0)
            {
                ...
                for (int i = 0; i < wlayer1.Count; i++)//1F壁
                {
                    ...
                    for (int j = 0; j < shell.Length; j++)
                    {
                        ...
                        var width = 0.0; var theta1 = 90.0; var theta2 = 0.0; var alpha2 = Math.Min(maxW, alpha);
                ...
                if (qax != 0.0 && qay != 0.0) { Qax.Add(qax); Qay.Add(qay); }
                var beta = new List<double>(); var gamma = windW / 100.0;
                ...
                {
                    beta.Add(eathquakeW[0] / 100.0);
                }
                else if (flayer3.Count == 0)
                {
                    beta.Add(eathquakeW[0] / 100.0); beta.Add(eathquakeW[1] / 100.0);
                }
                else
                {
                    beta.Add(eathquakeW[0] / 100.0); beta.Add(eathquakeW[1] / 100.0); beta.Add(eathquakeW[2] / 100.0);
                }
                var q = new List<Brep>();
        ...
        protected override System.Drawing.Bitmap Icon
        {
            get
            {
                ...
                return OpenSeesUtility.Properties.Resources.kabecheck2025;
            }
        }
        ...
        {
            get { return new Guid("306783CC-9633-4628-AFF2-E4235AC5E84B"); }
        }
        ///ここからカスタム関数群********************************************************************************
