...
namespace VisualizeResult
{
    public class VisualizeResult : GH_Component
    {
        ...
        public VisualizeResult()
          : base("VisualizeAnalysisResult", "VisualizeResult",
                ...
                else if (_sec_f.Branches[0][0].Value != -9999)
                {
                    ...
                    else if (l_vec[0] != new Vector3d(-9999, -9999, -9999))
                    {
                        ...
                        for (int ind = 0; ind < index.Count; ind++)
                        {
                            ...
                            for (int i = 0; i < 3; i++)
                            {
                                Qymax = Math.Max(Qymax, Math.Abs(sec_f[e][i * 6 + 1].Value) * nscale); Qzmax = Math.Max(Qzmax, Math.Abs(sec_f[e][i * 6 + 2].Value) * nscale); Nmin = Math.Min(Nmin, sec_f[e][i * 6].Value * nscale);
                        ...
                        for (int ind = 0; ind < index.Count; ind++)
                        {
                            ...
                            if (on_off3_22 == 1)///My
                            {
                                ...
                                for (int i = 0; i < pts.Length - 1; i++)
                                {
                                    ...
                                    var mesh = new Mesh();
                                    mesh.Vertices.Add(pts[i]); mesh.Vertices.Add(pts2[i]);
                                    if (val1 * val2 >= 0)
                                    {
                                        mesh.Vertices.Add(pts2[i + 1]); mesh.Vertices.Add(pts[i + 1]);
                                        mesh.VertexColors.Add(color1); mesh.VertexColors.Add(color1); mesh.VertexColors.Add(color2); mesh.VertexColors.Add(color2);
                                    ...
                                    else
                                    {
                                        ...
                                        mesh.Vertices.Add(ptc);
                                        mesh.Vertices.Add(pts[i + 1]); mesh.Vertices.Add(pts2[i + 1]);
                                        mesh.VertexColors.Add(color1); mesh.VertexColors.Add(color1); mesh.VertexColors.Add(color3);
                                        mesh.VertexColors.Add(color2); mesh.VertexColors.Add(color2);
                                        mesh.Faces.AddFace(0, 1, 2); mesh.Faces.AddFace(2, 3, 4);
                                        _mesh.Add(mesh);
                                    }
                                }
                                if (Value == 1)
                            ...
                            {
                                _Nvalue.Add(Nc);
                                if (Nc > 0) { _c3.Add(Color.Blue); }
                                else { _c3.Add(Color.Red); }
                                var vec = (r2 - r1) * Nc * nscale / (r2 - r1).Length;
                                _p1.Add(rc - vec * 0.5); _p2.Add(rc + vec * 0.5);
                                if (Value == 1)
                                {
                                    _funit.Add(unit_of_force); _lunit.Add("");
                                }
                            }
                            if (on_off3_21 == 1)///Mx
                            {
                                _Nvalue.Add(Mxc);
                                _c3.Add(Color.DarkGoldenrod);
                                var vec = (r2 - r1) * Mxc * mscale / (r2 - r1).Length;
                                _p1.Add(rc - vec * 0.5); _p2.Add(rc + vec * 0.5);
                                if (Value == 1)
                                {
                                    _funit.Add(unit_of_force); _lunit.Add(unit_of_length);
                                }
                            }
                        }
        ...
        protected override System.Drawing.Bitmap Icon
        {
            get
            {
                ...
                return OpenSeesUtility.Properties.Resources.VisResult;
            }
        }
        ...
        {
            get { return new Guid("cdfa5fdf-f9de-46f0-b0d9-61c047fdb8aa"); }
        }
        ///ここからカスタム関数群********************************************************************************
